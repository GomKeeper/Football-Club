          ...
          <Dialog.Panel className="w-full max-w-lg rounded-2xl bg-white shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 border-b flex justify-between items-center bg-gray-50 rounded-t-2xl">
              <div>
                 ...
                 <p className="text-xs text-gray-500">참석 {roster.ATTENDING.length} · 불참 {roster.ABSENT.length} · 미정 {roster.PENDING.length}</p>
              </div>
              <button onClick={onClose}><XMarkIcon className="w-6 h-6 text-gray-400 hover:text-black" /></button>
            </div>
            ...
            <div className="flex border-b text-sm font-medium text-gray-500">
               {['ATTENDING', 'ABSENT', 'PENDING'].map((status) => (
                 ...
                 >
                   {status === 'ATTENDING' ? '참석' : status === 'ABSENT' ? '불참' : '미정'} ({roster[status as keyof typeof roster].length})
                 </button>
               ))}
            </div>
            ...
            <div className="overflow-y-auto p-4 space-y-2 flex-1">
               {roster[activeTab].length === 0 ? (
                 ...
                 roster[activeTab].map((p) => (
                   <div key={p.id} className="flex justify-between items-center p-3 bg-white border rounded-xl hover:border-gray-300 transition-colors group">
