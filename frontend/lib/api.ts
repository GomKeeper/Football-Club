...
export async function getMyParticipation(matchId: number) {
  const token = localStorage.getItem('token'); // Assuming you store JWT here
  if (!token) return null;

  const response = await fetch(`${API_URL}/participations/matches/${matchId}/me`, {
    headers: { Authorization: `Bearer ${token}` },
  });

  if (!response.ok) return null;
  // Handle empty response (null) if user hasn't voted
...
export async function loginWithBackend(payload: KakaoLoginPayload) {
  ...
  // 4. CRITICAL: Store the BACKEND token, not the Kakao token
